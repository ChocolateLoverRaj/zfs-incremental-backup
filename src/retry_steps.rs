...
pub trait StateSaver<State, SaveError> {
    async fn save_state<'a>(&'a mut self, state: &'a State) -> Result<(), SaveError>;
}
...
pub async fn retry_with_steps<State, StepError, StepDoerType, StateSaverType, SaveError, Output>(
    mut state: State,
    mut step_doer: StepDoerType,
    mut save_fn: StateSaverType,
...
{
    Ok(loop {
        match step_doer.do_step(state, &mut save_fn).await? {
